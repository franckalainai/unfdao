                ...
                <ul class="categories">
                    @foreach ($categories as $category)
                        <li>
                            ...
                            <span class="badge pull-right">{{ ($tmp = $category->posts) ? $tmp->count() : 0 }}</span>
                        </li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="widget-body">
                <ul class="categories">
                    @foreach($archives as $archive)
                        <li>
                            <a href="{{ route('archives', ['month' => $archive->month, 'year' => $archive->year]) }}">{{ month_name($archive->month) . " " . $archive->year }}</a>
